...
// } from "cc";
// import { EDITOR, EDITOR_NOT_IN_PREVIEW } from "cc/env";
// import { MeshSprite } from "../../curvetexture/MeshSprite";
...
//     private _cachedGizmoNodes: Node[] = [];
//     private _lastGizmoCount: number = 0;
...
//             }
//             console.warn(
//                 IN_EDITOR_ZH ? `${this.node.name} 控制点有自相交,请检查! ` : " (Control points have self-intersection, please check!)"
//             );
//             return;
//         }
...
//             console.warn(
//                 IN_EDITOR_ZH ? `${this.node.name} 控制点有重复点,请检查!` : " (Control points have duplicate points, please check!)"
//             );
//             return;
//         }
...
//             console.warn(
//                 IN_EDITOR_ZH ? "纹理的高度不是2的指数,请检查!" : " (The height of the texture is not a power of 2, please check!)"
//             );
//             return;
//         }
...
//             _seg.push(...points);
//             physics ||
//                 console.log(this.node.name + (IN_EDITOR_ZH ? " 细分后点数量为:" : " (Number of points after subdivision):"), _seg.length);
...
//         const uvs = this._renderUVMode == TerrainRenderUVMode.UV_REPEAT_GRID ? this._calculateUVs() : this._calculateTangentsUVs();
...
//         // if (this._updateTimer < this._updateInterval && !this._needsUpdate) {
//         //     return;
//         // }
//         // this._updateTimer = 0;
...
//             await this._handleEditorMode();
//         }

//         this._handleGizmoNodes();
//         this._handlePositionChanges();
//     }

//     private _handleSyncMode(): void {
...
//     private async _handleEditorMode(): Promise<void> {
...
//     private _handleGizmoNodes(): void {
//         if (this._lastGizmoCount !== this.node.children.length) {
//             this._editNodes.length = 0;
//             this._editNodes = this.node.children.filter((item) => item.name.startsWith("gizmo_"));
//             this._lastGizmoCount = this.node.children.length;
//         }
...
//         this._updateGizmoVisuals();
//         this._updateGizmoAngles();
//     }

//     private _updateGizmoVisuals(): void {
...
//             if (uit.contentSize.width !== 80) uit.setContentSize(80, 80);
//             if (opa.opacity !== 255) opa.opacity = 255;
//             if (!sp.color.equals(this.gizmoColor)) sp.color = this.gizmoColor;
//             if (sp.spriteFrame !== this._gizmotag) sp.spriteFrame = this._gizmotag;
...
//     private _updateGizmoAngles(): void {
//         if (this._sortInXDirection) {
...
//     private _handlePositionChanges(): void {
//         let str = "";
//         this._editNodes.forEach((item) => {
