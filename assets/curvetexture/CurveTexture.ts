...
export class CurveTexture extends Component {
  ...
  set syncTarget(value) {
    if (this._syncOnOff) {
      if (value) {
        ...
        if (!c) {
          console.warn(IN_EDITOR_ZH ? '跟随目标节点没有CurveTexture组件,请添加' : ' (The target node to follow does not have a CurveTexture component, please add it)');
          return;
        }
        if (value === this.node) {
        ...
        if (c.syncTarget) {
          if (c.syncTarget === this.node) {
            console.warn(IN_EDITOR_ZH ? '出现循环跟随啦,非法操作!' : ' (Circular following detected, illegal operation!)');
            return;
          }
        }
      }
  ...
  @property({
    ...
    visible(this: CurveTexture) {
      return this._renderMode == TerrainRenderMode.TANGENT_MODE || this._renderMode == TerrainRenderMode.TANGENT_MODE_CENTER;
    },
  })
  ...
  @property({})
  private _flowSpeed: number = 0.0;
  @property({
    tooltip: IN_EDITOR_ZH ? 'uv横向流动速度' : ' (Horizontal UV flow speed)',
    displayName: IN_EDITOR_ZH ? 'uv横向流动速度' : 'Horizontal UV flow speed',
    group: IN_EDITOR_ZH ? '渲染属性' : '(Terrain Rendering)',
    range: [-100, 100, 0.1],
    visible(this: CurveTexture) {
      return this._renderMode == TerrainRenderMode.TANGENT_MODE || this._renderMode == TerrainRenderMode.TANGENT_MODE_CENTER;
  ...
  @property({})
  private _textureColor: Color = new Color('#FFFFFF');
  @property({
    type: Color,
    tooltip: IN_EDITOR_ZH ? '纹理颜色,包括首尾纹理' : '(Texture color, including head and tail textures)',
    displayName: IN_EDITOR_ZH ? '纹理颜色' : ' (Texture Color)',
    group: IN_EDITOR_ZH ? '渲染属性' : ' (Terrain Rendering)',
  })
  get textureColor() {
  ...
  set textureColor(value) {
    this._textureColor.set(value.r, value.g, value.b, value.a);
    this._refreshAll();
  }
  ...
  @property({
    tooltip: IN_EDITOR_ZH ? '地形的厚度,填0是设置为纹理高度值' : ' (Thickness of the terrain, 0 sets it to the texture height value)',
  ...
  @property({
    type: Vec3,
    tooltip: IN_EDITOR_ZH ? '渲染起始的偏移值,z暂时无效' : ' (Offset value for rendering start, z is temporarily invalid)',
    group: IN_EDITOR_ZH ? '渲染属性' : ' (Terrain Rendering)',
  ...
  set controlPoints(value) {
    ...
    for (let i = 0; i < this._controlPoints.length; i++) {
      ...
      const epsilon = 0.001; // 设置阈值
      if (Math.abs(point.x) < epsilon) point.x = 0;
      if (Math.abs(point.y) < epsilon) point.y = 0;
      if (Math.abs(point.z) < epsilon) point.z = 0;
      // 对精度进行控制，最多小数点后三位
  ...
  private _controlPointsHashLast: number = 0;
  private _calcControlPointsHash(): number {
    return murmurhash2_32_gc(this._controlPoints.map((p) => `${p.x},${p.y},${p.z}`).join('|'), 12345);
  }
  private _attrsHashLast: number = 0;
  private _calcAttrsAHash(): number {
    const s = this._controlPoints.map((p) => `${p.x},${p.y},${p.z}`).join('|') + this.smoothness.toString();
    return murmurhash2_32_gc(s, 12345);
  }
  ...
  @property({
    type: CCInteger,
    tooltip: IN_EDITOR_ZH ? '每两控制点之间细分的数量,默认16,如果是0,则不细分' : ' (Number of subdivisions between every two control points, default is 16)',
  ...
  @property({})
  private _minSegmentLength: number = 4;
  @property({
    type: CCInteger,
    tooltip: IN_EDITOR_ZH ? '细分时细分点之间的距离阈值' : ' (Distance threshold between subdivided points during subdivision)',
    displayName: IN_EDITOR_ZH ? '细分点距离阈值' : ' (Subdivided Point Distance Threshold)',
    group: IN_EDITOR_ZH ? '渲染属性' : ' (Terrain Rendering)',
    slide: true,
    range: [2, 64, 2],
  })
  ...
  set minSegmentLength(value) {
    this._minSegmentLength = value % 2 == 1 ? value + 1 : value;
    this._refreshAll();
  }

  @property({})
  private _minControlLength: number = 32;
  @property({
    type: CCInteger,
    tooltip: IN_EDITOR_ZH ? '细分前控制点之间的距离阈值' : ' (Distance threshold between control points before subdivision)',
    displayName: IN_EDITOR_ZH ? '细分前控制点距离阈值' : ' (Control Point Distance Threshold)',
    group: IN_EDITOR_ZH ? '渲染属性' : ' (Terrain Rendering)',
    slide: true,
    range: [8, 256, 1],
  })
  get minControlLength() {
    return this._minControlLength;
  }
  set minControlLength(value) {
    this._minControlLength = value;
    this._refreshAll();
  }
  //
  @property({})
  ...
  @property({
    tooltip: IN_EDITOR_ZH ? '是否首尾闭合,渲染为一个闭合的区域' : ' (Whether the head and tail are closed, rendered as a closed area)',
  ...
  set headTailCloseOnoff(value) {
    ...
    if (result2.result) {
      ...
      console.warn(IN_EDITOR_ZH ? `${this.node.name} 控制点有自相交,请检查! ` : ' (Control points have self-intersection, please check!)');
      return;
    }
    ...
    if (result1.result) {
      console.warn(
        IN_EDITOR_ZH
          ? `${this.node.name} 控制点有连续共线点,请检查! 点索引:${result1.index}/${result1.index + 1}/${result1.index + 2}`
          : ' (Control points have continuous collinear points, please check!)'
      );
      return;
    }
    if (UsefulTools.hasDuplicatePoints(this._controlPoints)) {
      console.warn(IN_EDITOR_ZH ? `${this.node.name} 控制点有重复点,请检查!` : ' (Control points have duplicate points, please check!)');
      return;
    }
  ...
  @property({
    tooltip: IN_EDITOR_ZH ? '闭合后内部渲染组件,如果不需要可以不设置' : ' (Inner rendering component after closure, if not needed, it can be left unset)',
  ...
  @property({
    type: CCFloat,
    tooltip: IN_EDITOR_ZH ? '物理碰撞器的厚度,填0是设置为纹理高度值' : ' (Thickness of the physical collider, 0 sets it to the texture height value)',
  ...
  @property({
    type: Vec3,
    tooltip: IN_EDITOR_ZH ? '物理碰撞器的偏移值,默认(0,0,0)' : ' (Offset value for the physical collider, default is (0,0,0))',
  ...
  //有时候更改了属性,需要强制渲染一次
  //Sometimes when changing properties, a forced render is needed.
  ...
  onFocusInEditor(): void {
    ...
    // const result = JSON.stringify(this._vertexesSegment);
    // console.log(`export data[${timestamp}]: ${result}`);
    // if (EDITOR) {
    //   //@ts-ignore
    //   Editor.Clipboard.write('text', `${result}`);
    //   console.warn('已复制路径数据到你的剪贴板,可以直接粘贴到你需要的地方');
    // }
  }
  ...
  protected onEnable() {
    ...
    this._initDynamicMesh();
    this._registerKeyForGizmo();
    this._refreshAll('565');
  }

  protected onDestroy(): void {
    this._deInitDynamicMesh();
  }

  // 动态mesh缓存 'Dynamic mesh cache'
  private _dynamicMesh: Mesh = null;
  private _dynamicGeo: primitives.IDynamicGeometry = null;
  @property({})
  private _maxVertexCount: number = 1024;
  @property({
    tooltip: IN_EDITOR_ZH
      ? '预申请的渲染顶点数量 <br>如果你的控制点会在运行中的动态改变,则需要确立一个最大渲染顶点数量(一般大于控制点数量的两倍)'
      : 'Pre-allocated render vertex count <br> If your control points may change dynamically during runtime, you need to define a **maximum render vertex count** (typically more than twice the number of control points).',
    displayName: IN_EDITOR_ZH ? '预申请渲染顶点数量' : 'Pre-allocated Rendering Vertex Count',
    group: IN_EDITOR_ZH ? '渲染属性' : '(Terrain Rendering)',
    range: [4, 4096, 2],
    // displayOrder: 0,
  })
  ...
  // 初始化动态mesh 'Initialize dynamic mesh'
  private _initDynamicMesh() {
    this._maxIndexCount = this._maxVertexCount * 3;
    if (!this._dynamicMesh) {
      const options = {
        maxSubMeshes: 1,
        maxSubMeshVertices: this._maxVertexCount,
        maxSubMeshIndices: this._maxIndexCount,
      };
      this._dynamicGeo = {
        positions: new Float32Array(options.maxSubMeshVertices * 3),
        uvs: new Float32Array(options.maxSubMeshVertices * 2),
        indices16: new Uint16Array(options.maxSubMeshIndices),
        minPos: { x: 0, y: 0, z: 0 },
        maxPos: { x: 0, y: 0, z: 0 },
      };
      this._dynamicMesh = utils.MeshUtils.createDynamicMesh(0, this._dynamicGeo, undefined, options);
      this.renderer.mesh = this._dynamicMesh;
    }
  }

  private _deInitDynamicMesh() {
    this._dynamicMesh.destroy();
    this._dynamicMesh = null;
    this._dynamicGeo.positions = null;
    this._dynamicGeo.uvs = null;
    this._dynamicGeo.indices16 = null;
  }
  ...
  private _refreshAll(from: string = '') {
    ...
    if (!this.renderer) {
      console.warn(IN_EDITOR_ZH ? '没有MeshRenderer组件,请添加MeshRenderer组件!' : ' (No MeshRenderer component, please add MeshRenderer component!)');
      return;
    }
    ...
    if (this._curve_width & (this._curve_width - 1)) {
      console.warn(IN_EDITOR_ZH ? '纹理的宽度不是2的指数,请检查!' : ' (The width of the texture is not a power of 2, please check!)');
      return;
    }
    if (this._curve_height & (this._curve_height - 1)) {
      console.warn(IN_EDITOR_ZH ? '纹理的高度不是2的指数,请检查!' : ' (The height of the texture is not a power of 2, please check!)');
      return;
    }

    if (this.gizmoTag == null) {
      console.warn(IN_EDITOR_ZH ? '还未指定[ 控制点纹理 ]! 拖入 [ gizmotag ] 资源即可' : ' ([ Control Point Texture] has not been specified yet! Drag in the [gizmotag] resource)');
      // return;
    }
    ...
    const curAttrsHash = this._calcAttrsAHash();
    if (this._attrsHashLast != curAttrsHash) {
      needUpdate = true;
      this._attrsHashLast = curAttrsHash;
    }
  ...
  // private _setFlowSpeed() {
  //   let mat = this.renderer.getMaterialInstance(0) as Material;
  //   if (!mat) return;
  //   mat.setProperty('flowSpeed', this._flowSpeed, 0);
  // }
  ...
  private _applySpriteFrame() {
    if (this._spFrame && this.renderer) {
      ...
      mat.setProperty('textureColor', this._textureColor, 0);
      mat.setProperty('flowSpeed', this._flowSpeed, 0);
    } else {
      if (!this._spFrame) {
        let mat = this.renderer!.getMaterialInstance(0) as Material;
  ...
  private _getTangentPos(current: Vec3, next: Vec3, angleRad: number, thickness: number): Vec3 {
    const v1 = next.clone().subtract(current).normalize().toVec2().rotate(angleRad).multiplyScalar(thickness).add(current.toVec2());
    return v1.toVec3();
  }
  ...
  private _dynamicMeshHead: Mesh = null;
  private _dynamicGeoHead: primitives.IDynamicGeometry = null;
  private _updateHead() {
    if (this._headTailFixedOnOff) {
      ...
      // meshR.mesh = mesh;
      const vCount = 4;
      const iCount = vCount * 3;
      if (!this._dynamicMeshHead) {
        const options = {
          maxSubMeshes: 1,
          maxSubMeshVertices: vCount,
          maxSubMeshIndices: iCount,
        };
        this._dynamicGeoHead = {
          positions: new Float32Array(options.maxSubMeshVertices * 3),
          uvs: new Float32Array(options.maxSubMeshVertices * 2),
          indices16: new Uint16Array(options.maxSubMeshIndices),
          minPos: { x: 0, y: 0, z: 0 },
        ...
        this._dynamicMeshHead = utils.MeshUtils.createDynamicMesh(0, this._dynamicGeoHead, undefined, options);
        meshR.mesh = this._dynamicMeshHead;
      }
      const vertexCount = tmp.length;
      // const indices = this._calculateIndices();
      const indexCount = indices.length;
      // const uvs = uvs;

      // 写入动态geo 'Write to dynamic geometry'
      const geo = this._dynamicGeoHead;
      // positions
      for (let i = 0; i < vertexCount; i++) {
        geo.positions[i * 3] = tmp[i].x;
        geo.positions[i * 3 + 1] = tmp[i].y;
        geo.positions[i * 3 + 2] = 0;
      }
      // uvs
      for (let i = 0; i < vertexCount; i++) {
        geo.uvs[i * 2] = uvs[i * 2];
        geo.uvs[i * 2 + 1] = uvs[i * 2 + 1];
      }
      // indices
      ...
      // 清理多余部分
      for (let i = vertexCount * 3; i < geo.positions.length; i++) geo.positions[i] = 0;
      for (let i = vertexCount * 2; i < geo.uvs.length; i++) geo.uvs[i] = 0;
      for (let i = indexCount; i < geo.indices16.length; i++) geo.indices16[i] = 0;
      ...
      // geo.minPos.set(minX, minY, minZ);
      // geo.maxPos.set(maxX, maxY, maxZ);
      geo.minPos.x = minX;
      geo.minPos.y = minY;
      geo.minPos.z = minZ;
      geo.maxPos.x = maxX;
      ...
      // 更新mesh
      this._dynamicMeshHead.updateSubMesh(0, geo);
      meshR.onGeometryChanged();
    } else {
      let headNode = this.node.getChildByName('_headNode_');
      if (headNode) {
        headNode.destroy();
  ...
  private _dynamicMeshTail: Mesh = null;
  private _dynamicGeoTail: primitives.IDynamicGeometry = null;
  private _updateTail() {
    if (this._headTailFixedOnOff) {
      if (!this._headTailFrame) {
        console.warn(IN_EDITOR_ZH ? '请指定对应的首尾纹理' : ' (Please set the head/tail texture)');
      ...
      let p1 = this._getTangentPos(p0, this._vertexesSegment[this._vertexesSegment.length - 2], Math.PI / 2, this._curve_height_result);
      let p2 = p0
        .clone()
      ...
      const vCount = 4;
      const iCount = vCount * 3;
      if (!this._dynamicMeshTail) {
        const options = {
          maxSubMeshes: 1,
          maxSubMeshVertices: vCount,
          maxSubMeshIndices: iCount,
        };
        this._dynamicGeoTail = {
          positions: new Float32Array(options.maxSubMeshVertices * 3),
          uvs: new Float32Array(options.maxSubMeshVertices * 2),
          indices16: new Uint16Array(options.maxSubMeshIndices),
          minPos: { x: 0, y: 0, z: 0 },
        ...
        this._dynamicMeshTail = utils.MeshUtils.createDynamicMesh(0, this._dynamicGeoTail, undefined, options);
        meshR.mesh = this._dynamicMeshTail;
      }
      ...
      // 写入动态geo 'Write to dynamic geometry'
      const geo = this._dynamicGeoTail;
      // positions
      for (let i = 0; i < vertexCount; i++) {
        geo.positions[i * 3] = tmp[i].x;
        geo.positions[i * 3 + 1] = tmp[i].y;
      ...
      // 更新mesh
      this._dynamicMeshTail.updateSubMesh(0, geo);
      meshR.onGeometryChanged();
    } else {
  ...
  // private _minSegmentLength: number = 2;
  private _generateSegmentVertex(seg_count: number = 8, physics: boolean = false): Vec3[] {
      ...
      if (EDITOR) {
        physics ||
          console.log(
            this.node.name +
              (IN_EDITOR_ZH
                ? ` 细分后控制点数量为:${_seg.length},maxVertexCount 推荐值为:${_seg.length * 2},如果渲染不完全,请修改这个参数`
                : ` (Number of control points after subdivision):${_seg.length * 2},If rendering is incomplete, modify this parameter.`)
          );
      }
    ...
    for (let i = 0; i < interCounts; i++) {
      ...
      if (dist < this._minControlLength) {
        seg = 1;
      } else {
  ...
  private _updateMesh() {
    if (!this._spFrame || this._controlPoints.length < 2) {
      console.warn(IN_EDITOR_ZH ? '至少需要两个控制点' : 'Need at least 2 points to define the curve.');
    ...
    // 写入动态geo 'Write to dynamic geometry'
    const geo = this._dynamicGeo;
    // positions
    for (let i = 0; i < vertexCount; i++) {
      geo.positions[i * 3] = this._vertexes[i].x;
      geo.positions[i * 3 + 1] = this._vertexes[i].y;
      geo.positions[i * 3 + 2] = 0;
    }
    // uvs
    for (let i = 0; i < vertexCount; i++) {
    ...
    // 更新mesh
    this._dynamicMesh.updateSubMesh(0, geo);
   ...
   */
  private _crossPointBy2Segment(p1: Vec2 | Vec3, p2: Vec2 | Vec3, p3: Vec2 | Vec3, p4: Vec2 | Vec3, extended: boolean = false): Vec2 {
    const a = p1,
      b = p2; // 第一条线段: a->b
    const c = p3,
      d = p4; // 第二条线段: c->d
    ...
    for (let index = 0; index < temp.length; index++) {
      ...
      if (index < temp.length - 1) {
        ...
        if (index > 0) {
          ...
          //计算顺时针交点
          let crossPointClock = this._crossPointBy2Segment(pCurrentClockLast, pNextClockLast, pCurrentClock, pNextClock, true);
  ...
  private _calclateTangentCenterPoints(indata: Vec3[], thickness: number, physics: boolean = false): Vec3[] {
    ...
    for (let index = 0; index < temp.length; index++) {
      ...
      if (index < temp.length - 1) {
        ...
        if (index > 0) {
          //计算逆时针交点
          let crossPointAnticlock = this._crossPointBy2Segment(pCurrentAnticlockLast, pNextAnticlockLast, pCurrentAnticlock, pNextAnticlock, true);
  ...
  @property({
    type: Color,
    tooltip: IN_EDITOR_ZH ? '编辑控制点时的gizmo颜色,alpha值最小值为50' : ' (Gizmo color when editing control points, alpha value is >=50)',
  ...
  protected async update(dt: number): Promise<void> {
    ...
    if (EDITOR_NOT_IN_PREVIEW) {
      ...
      if (this._lastEditNodes != null) {
        const newNodes = t.filter((node) => this._lastEditNodes.indexOf(node) === -1).filter((node) => node.name.startsWith('gizmo_'));
